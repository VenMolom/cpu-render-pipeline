...
{
    public class Cylinder : RenderObject3D
    {
        private float h = 1f;
        private int n = 5;
        private float r = 1f;
        ...
        public Cylinder(Scene scene)
        {
            Control = new CylinderControl(this, scene);
            SetVectors();
        }
        ...
        protected override void SetVectors()
        {
            ...
            for (int j = 0; j < n; ++j)
            {
                ...
                SetTexCoord(currentTriangle, 0, 0, phi);
                SetTexCoord(currentTriangle, 1, third, phi);
                SetTexCoord(currentTriangle++, 2, third, phi2);
            }
            ...
            for (int j = 0; j < n; ++j)
            {
                ...
                var v1 = GetSphereVertice(phi, height);
                var v2 = GetSphereVertice(phi2, height);
                var v3 = GetSphereVertice(phi2, -height);
                var v4 = GetSphereVertice(phi, -height);

                SetTriangeVectors(currentTriangle, v2, v1, v4);

                SetTexCoord(currentTriangle, 0, third, phi2);
                SetTexCoord(currentTriangle, 1, third, phi);
                SetTexCoord(currentTriangle++, 2, 1f - third, phi);

                SetTriangeVectors(currentTriangle, v3, v2, v4);

                SetTexCoord(currentTriangle, 0, 1f - third, phi2);
                SetTexCoord(currentTriangle, 1, third, phi2);
                SetTexCoord(currentTriangle++, 2, 1f - third, phi);
            }
            ...
            for (int j = 0; j < n; ++j)
            {
                ...
                SetTexCoord(currentTriangle, 0, 1f - third, phi);
                SetTexCoord(currentTriangle, 1, 1f, phi);
                SetTexCoord(currentTriangle++, 2, 1f - third, phi2);
            }
        }
        ...
        private void SetTriangeVectors(int triangleIndex, Vector3 v1, Vector3 v2, Vector3 v3)
        {
            vertices[triangleIndex, 0] = new Vector4(v1, 1f);
            vertices[triangleIndex, 1] = new Vector4(v2, 1f);
            vertices[triangleIndex, 2] = new Vector4(v3, 1f);

            normals[triangleIndex, 0] = new Vector4(v1.X, 0, v1.Z).Normalized;
            normals[triangleIndex, 1] = new Vector4(v2.X, 0, v2.Z).Normalized;
            normals[triangleIndex, 2] = new Vector4(v3.X, 0, v3.Z).Normalized;

            tangents[triangleIndex, 0] = new Vector4(0, 1f, 0);
            tangents[triangleIndex, 1] = new Vector4(0, 1f, 0);
            tangents[triangleIndex, 2] = new Vector4(0, 1f, 0);

            binormals[triangleIndex, 0] = new Vector4(Vector3.Cross(Vector3.UWorld, new Vector3(v1.X, 0, v1.Z).Normalized));
            binormals[triangleIndex, 1] = new Vector4(Vector3.Cross(Vector3.UWorld, new Vector3(v2.X, 0, v2.Z).Normalized));
            binormals[triangleIndex, 2] = new Vector4(Vector3.Cross(Vector3.UWorld, new Vector3(v3.X, 0, v3.Z).Normalized));
        }
    }
}